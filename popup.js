...
function generateSentenceFormula(segments, detailsList) {
  ...
  segments.forEach((seg, idx) => {
    ...
    // Check for copulas
    const copulas = ['amp', 'です', 'だ', 'である', 'でした', 'だった'];
...
function mergeSegments(rawSegments, blacklist = []) {
  const particles = ['amp', 'は', 'が', 'を', 'に', 'へ', 'で', 'と', 'も', 'の', 'か', 'ね', 'よ', 'から', 'まで', 'より', 'amp', 'だけ', 'ばかり', 'ほど', 'ぐらい', 'など', 'て', 'た', 'だ', 'です', 'である', 'にぇ', 'ね', 'よ', 'な', 'わ', 'ぞ', 'ぜ', 'なら'];
    ...
    const uniqueWords = [...new Set(
      segments
        ...
        .filter(w => {
          ...
          if (['amp', 'です', 'だ', 'である', 'でした', 'だった'].includes(w)) return false;
